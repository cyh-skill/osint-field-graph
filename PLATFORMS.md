...
| Chinese tech / 算法 / 内容型技术大V | **知乎** follows (+ GitHub/PyPI to verify) | `cmd:` or `--edges` | web-access: a 垂类榜单 / your own 关注 + doc authors as seeds |
| ML/AI research-that-lives-on-X, crypto, indie hackers | **X / Twitter** follows | `--edges` | web-access: a curated List, or top accounts of the niche |
| 种草 / 美妆 / 时尚 / 生活方式 creator | **小红书 / 微博 / 抖音 / B站** follows | `--edges` | web-access + 千瓜/蝉妈妈/新榜 垂类榜单 as seeds |
| founders / VC / operators / a business vertical | **LinkedIn / X** | `--edges` | web-access: Crunchbase list, a "top N founders" list, portfolio pages |
| writers / journalists / newsletter | **Substack** recommendations / X / 知乎 | `cmd:`/`--edges` | web-access: Substack leaderboard, masthead, a topic's top writers |
...
- **Scriptable but not built-in → `cmd:`.** If you can write one command that, given
  ...
  drop it in as `--provider 'cmd:<cmd with {}>'` — no engine change.
- **No → harvest with the web-access skill, then `--edges`.** X, 小红书, 微博, B站,
  LinkedIn, 知乎 all block static scraping and need a logged-in browser. Use the
  **web-access skill** to walk each seed's follow list into a `seed<TAB>neighbor`
...
2. With the **web-access skill**, for each seed open its *following* page and
...
- **知乎** — the source article's platform. `/api/v4/members/{id}` (with login)
  ...
  web-access. Verify real names via GitHub/PyPI/arXiv (`SOURCES.md`).
